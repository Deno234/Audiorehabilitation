...
Ovdje ne govorimo o rezultatima, nego o tome što su tri sintetizatora zapravo.

eSpeak NG je klasični rule-based, odnosno formantni sintetizator. On ne koristi neuronsku mrežu, nego pravila izgovora i prozodije za hrvatski glas `hr`. Prednost mu je jednostavnost, brzina i reproducibilnost.

Coqui VITS HR je neuronski end-to-end TTS model. VITS spaja akustičko modeliranje i neuralnu sintezu u jedan sustav: iz teksta nastaje latentni akustički prikaz, a decoder/vocoder proizvodi waveform. U konfiguraciji koristimo `tts_models/hr/cv/vits` preko lokalne skripte `coqui_tts_no_cache.py`.

SpeechT5 HR je neuronski encoder-decoder pristup s odvojenim vocoderom. Tekst prolazi kroz SpeechT5 processor i model, izlaz je uvjetovan speaker embeddingom, a HiFi-GAN vocoder pretvara akustički prikaz u zvuk. Koristimo lokalne foldere `models/speecht5_tts_hr`, `models/speecht5_processor`, `models/speecht5_hifigan` i x-vector speaker embedding.

Naglasak: adapter je samo programski omotač u pipelineu. Model je stvarni sintetizator. Usporedba je fer jer sva tri dobivaju isti validirani tekst i svi izlazi se normaliziraju na isti WAV format.
